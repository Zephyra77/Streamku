...
open class SoraStream : TmdbProvider() {
    ...
    private fun getImageUrl(link: String?): String? = link?.let { if (it.startsWith("/")) "https://image.tmdb.org/t/p/w500/$it" else it }
    private fun getOriImageUrl(link: String?): String? = link?.let { if (it.startsWith("/")) "https://image.tmdb.org/t/p/original/$it" else it }
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = app.get("${request.data}$adultQuery&page=$page")
            .parsedSafe<Results>()?.results?.mapNotNull { media -> media.toSearchResponse(type) }
    ...
    private fun Media.toSearchResponse(type: String? = null): SearchResponse? {
        return newMovieSearchResponse(
            ...
            this.score = voteAverage?.let { Score.from10(it.toFloat()) }
        }
    }
}
...
override suspend fun search(query: String): List<SearchResponse>? {
    return app.get("$tmdbAPI/search/multi?api_key=$apiKey&language=en-US&query=$query&page=1&include_adult=${settingsForProvider.enableAdult}")
        .parsedSafe<Results>()?.results?.mapNotNull { media -> media.toSearchResponse() }
}
...
override suspend fun load(url: String): LoadResponse? {
    ...
    val score = res.vote_average?.let { Score.from10((it as? Double)?.toFloat() ?: 0f) }
    ...
    return if (type == TvType.TvSeries) {
            ...
            app.get("$tmdbAPI/${data.type}/${data.id}/season/${season.seasonNumber}?api_key=$apiKey")
                .parsedSafe<MediaDetailEpisodes>()?.episodes?.map { eps ->
                    newEpisode(
                        ...
                        this.score = eps.voteAverage?.let { Score.from10(it.toFloat()) }
                        this.description = eps.overview
                    }.apply { this.addDate(eps.airDate) }
                }
        }?.flatten() ?: listOf()
        ...
        newMovieLoadResponse(
            ...
            LinkData(
                data.id, res.external_ids?.imdb_id, res.external_ids?.tvdb_id,
                data.type, title = title, year = year, orgTitle = orgTitle,
                isAnime = isAnime,
                jpTitle = res.alternative_titles?.results?.find { it.iso_3166_1 == "JP" }?.title,
                airedDate = res.releaseDate ?: res.firstAirDate,
                isAsian = isAsian,
