...
class Dramaindo : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        doc.select("div.st-resolusi ul.resolusi-list li").forEach { attr ->
            val decoded = base64Decode(attr.attr("data"))
            val json = JSONObject(decoded)
            val subUrl = json.optString("subtitle_url", "")
            if (subUrl.isNotBlank()) subtitleCallback(SubtitleFile("Indonesian", fixUrl(subUrl)))
            json.optJSONArray("links")?.let { arr ->
                for (i in 0 until arr.length()) {
                    val link = arr.getJSONObject(i).optString("url")
                    if (link.isNotBlank()) urls.add(fixUrl(link))
                }
            }
        }
        doc.select("iframe[src]").mapNotNull { it.attr("src") }.forEach { urls.add(it) }
        val validUrls = urls.filter { it.startsWith("http") }
        if (validUrls.isEmpty()) return@coroutineScope false
        Extractors.loadAllLinks(validUrls, str, subtitleCallback, callback)
        true
    }
