...
class IPTVSettingsFragment(private val plugin: IPTVPlugin) : BottomSheetDialogFragment() {
    private fun <T : View> View.findView(name: String): T {
        val id = plugin.resources!!.getIdentifier(name, "id", plugin.context.packageName)
        return this.findViewById(id)
    }

    private fun getLayout(name: String, inflater: LayoutInflater, container: ViewGroup?): View {
        val id = plugin.resources!!.getIdentifier(name, "layout", plugin.context.packageName)
        val layout = plugin.resources!!.getLayout(id)
        return inflater.inflate(layout, container, false)
    }

    private fun getDrawable(name: String): Drawable? {
        val id = plugin.resources!!.getIdentifier(name, "drawable", plugin.context.packageName)
        return ResourcesCompat.getDrawable(plugin.resources!!, id, null)
    }

    private fun getString(name: String): String? {
        val id = plugin.resources!!.getIdentifier(name, "string", plugin.context.packageName)
        return plugin.resources!!.getString(id)
    }

    private fun View.makeTvCompatible() {
        val outlineId = plugin.resources!!.getIdentifier("outline", "drawable", plugin.context.packageName)
    ...
    override fun onCreateView(
        ...
        addLinkButton.setOnClickListener {
            ...
            AlertDialog.Builder(context ?: throw Exception("Unable to build alert dialog"))
                ...
                .setPositiveButton("Save") { _, _ ->
                    ...
                    if (name.isEmpty() || !link.startsWith("http")) {
                        ...
                        try {
                            ...
                            if (existingLinks.any { it.name == name }) {
                                ...
                                val updatedLinks = existingLinks.toMutableList().apply { add(Link(name, link)) }
                                setKey("iptv_links", updatedLinks)
                                showToast("Save link success")
                                plugin.reload()
                            }
                        } catch (e: Exception) {
                            e.printStackTrace()
        ...
        listLinkButton.setOnClickListener {
