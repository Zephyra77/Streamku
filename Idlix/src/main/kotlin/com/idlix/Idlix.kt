...
class Idlix : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val req = if (nonPaged) app.get(request.data) else app.get("${url.first()}$page/?${url.lastOrNull()}")
        mainUrl = getBaseUrl(req.url)
        val document = req.document
        val home = (if (nonPaged) document.select("div.items.featured article") else document.select("div.items.full article, div#archive-content article")).mapNotNull { it.toSearchResult() }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val poster = images.shuffled().firstOrNull()?.selectFirst("a")?.attr("href") ?: document.select("div.poster > img").attr("src")
        val tags = document.select("div.sgeneros > a").map { it.text() }
        val year = Regex(",\\s?(\\d+)").find(document.select("span.date").text().trim())?.groupValues?.get(1).toString().toIntOrNull()
        ...
        val actors = document.select("div.persons > div[itemprop=actor]").map { Actor(it.select("meta[itemprop=name]").attr("content"), it.select("img").attr("src")) }
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        document.select("ul#playeroptionsul > li").map {
            ...
            if (!decrypted.contains("youtube")) loadExtractor(decrypted, directUrl, subtitleCallback, callback)
        }
    ...
    private fun String.fixBloat(): String = this.replace("\"", "").replace("\\", "")

    data class ResponseSource(@JsonProperty("hls") val hls: Boolean, @JsonProperty("videoSource") val videoSource: String, @JsonProperty("securedLink") val securedLink: String?)
    data class Tracks(@JsonProperty("kind") val kind: String?, @JsonProperty("file") val file: String, @JsonProperty("label") val label: String?)
    data class ResponseHash(@JsonProperty("embed_url") val embed_url: String, @JsonProperty("key") val key: String)
    data class AesData(@JsonProperty("m") val m: String)
}
